...
const CHARTS = [
  { rank:1, title:"Midnight Frequency", genre:"Lo-Fi", views:"124K" },
  { rank:2, title:"Golden Hour Drift", genre:"Ambient", views:"98K" },
  { rank:3, title:"Signal & Noise", genre:"Electronic", views:"87K" },
  { rank:4, title:"Soul Transmission", genre:"Soul", views:"76K" },
  { rank:5, title:"Open Road Jazz", genre:"Jazz", views:"61K" },
];
const SHORTS = [
  { title:"1AM Vibes", likes:"42K" }, { title:"Rain Loop #7", likes:"38K" },
  { title:"Sunrise Set", likes:"31K" }, { title:"Deep Focus", likes:"28K" },
];
...
/* ─── STATIC FLASH — imperative DOM, never causes rerender ─── */
function fireStaticFlash() {
  const el = document.createElement("div");
  el.style.cssText = `position:fixed;inset:0;z-index:200;pointer-events:none;
    background:repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(194,162,77,0.06) 2px,rgba(194,162,77,0.06) 4px);
    opacity:0.8;transition:opacity 0.4s ease-out;`;
...
function ChartRow({ item }) {
  ...
  return (
    <div onClick={()=>window.open(YT,"_blank")}
      ...
      <span style={{ color:C.deepGold,fontSize:13,fontFamily:"monospace",fontWeight:600 }}>{String(item.rank).padStart(2,"0")}</span>
      <div>
        <div style={{ color:C.cream,fontSize:13 }}>{item.title}</div>
        <div style={{ color:C.sageTeal,fontSize:11,marginTop:2 }}>{item.genre}</div>
      </div>
      <span style={{ color:C.dustyRose,fontSize:11 }}>{item.views}</span>
...
function ShortCard({ item }) {
  ...
  return (
    <div onClick={()=>window.open(YT,"_blank")}
      ...
      <div style={{ color:C.cream,fontSize:13,fontWeight:500,lineHeight:1.3 }}>{item.title}</div>
      <div style={{ display:"flex",alignItems:"center",gap:6 }}>
        <span style={{ color:C.gold,fontSize:11 }}>♥</span>
        <span style={{ color:C.dustyRose,fontSize:11 }}>{item.likes}</span>
      </div>
    </div>
  );
}
...
export default function AlecPennRadio() {
  ...
  const handleTuneIn = useCallback(()=>{
    ...
    const sweep = setInterval(()=>{
      f+=2.2; setFreq(parseFloat(f.toFixed(1)));
      if (f>=111.1) {
        clearInterval(sweep); setFreq(111.1);
        setTimeout(()=>{ setTuned(true); setTuning(false); }, 150);
      }
    },28);
  },[tuned,tuning]);
  ...
  return (
    <div style={{ minHeight:"100vh",background:C.black,color:C.cream,
      ...
      <nav style={{ position:"sticky",top:0,zIndex:50,
        ...
        <div style={{ flex:1,margin:"0 28px",overflow:"hidden",height:18,position:"relative" }}>
      ...
      <section id="broadcast" style={{ padding:"48px 32px 40px",maxWidth:1140,margin:"0 auto",position:"relative" }}>
        <div style={{
          ...
          boxShadow:tuned
            ?`0 0 90px ${C.teal}20,0 0 180px ${C.teal}0e,inset 0 1px 0 ${C.teal}33`
            :`0 0 60px ${C.deepGold}12,inset 0 1px 0 ${C.gold}18`,
          transform:`translateY(${scrollY*0.018}px)`,
          transition:"border-color 0.9s,box-shadow 0.9s" }}>

          <div style={{ position:"absolute",inset:0,zIndex:0 }}>
            <ParticleField active={tuned}/>
          </div>
          ...
          <div style={{ position:"relative",zIndex:2,padding:"36px 36px 32px",
            ...
            <div>
              <div style={{ marginBottom:10 }}>
                <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:11,
                  color:C.sageTeal,letterSpacing:"0.15em" }}>BROADCASTING SINCE 2024</span>
              </div>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:16,marginBottom:24 }}>
                <div style={{ display:"inline-flex",alignItems:"baseline",gap:4,
                  ...
                  boxShadow:tuned?`inset 0 0 24px ${C.teal}0e`:"none",
                  ...
                  <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:32,fontWeight:500,
                    ...
                    animation:tuned?"freq-flicker 3s ease infinite":"none",
                    transition:"color 0.3s" }}>{freq.toFixed(1)}</span>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:20,marginBottom:24 }}>
                ...
                <button onClick={handleTuneIn} disabled={tuning}
                  style={{ background:tuned?C.burgundy:C.teal,border:"none",color:C.cream,
                    ...
                    animation:tuned?"glow-pulse 2s ease infinite":"none",
            ...
            <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:20 }}>
                ...
                {[
                  ...
                  <div key={m.label} style={{ marginBottom:10 }}>
                    <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:9,
                      color:C.warmGold,letterSpacing:"0.15em" }}>{m.label}</span>
                    <div style={{ fontFamily:"monospace",fontSize:11,color:m.col,
                      letterSpacing:1,marginTop:3,transition:"color 0.6s" }}>{m.val}</div>
                  </div>
                ))}
                <div style={{ borderTop:`1px solid ${C.deepGold}22`,paddingTop:10,marginTop:4 }}>
                  <div style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:9,
                    color:C.warmGold,letterSpacing:"0.1em",marginBottom:4 }}>CATALOG</div>
              ...
              <a href="#" style={{ display:"block",width:"100%",textDecoration:"none",
                ...
                textAlign:"center",boxShadow:`0 4px 22px ${C.deepBurgundy}66`,
      ...
      <section id="charts" style={{ padding:"40px 32px",maxWidth:1140,margin:"0 auto" }}>
        <div style={{ display:"flex",alignItems:"baseline",gap:16,marginBottom:24 }}>
          ...
          <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:11,color:C.sageTeal }}>Most watched on YouTube</span>
        </div>
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:24 }}>
          <div style={{ border:`1px solid ${C.deepGold}28`,borderRadius:8,overflow:"hidden",background:"#0d0b09" }}>
            ...
            {CHARTS.map(item=><ChartRow key={item.rank} item={item}/>)}
            <div style={{ padding:"12px 16px",textAlign:"center" }}>
              <a href={YT} target="_blank" rel="noreferrer"
                style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:11,
                  color:C.sageTeal,textDecoration:"none",letterSpacing:"0.08em" }}>Full chart on YouTube ↗</a>
            </div>
          </div>
          ...
          <div style={{ border:`1px solid ${C.teal}44`,borderRadius:8,overflow:"hidden",
            ...
            <div style={{ padding:"10px 16px",
              ...
              fontSize:10,color:C.sageTeal,letterSpacing:"0.15em",
            ...
            <div style={{ flex:1,padding:24,display:"flex",flexDirection:"column",justifyContent:"space-between" }}>
              <div>
                <div style={{ fontFamily:"'DM Serif Display',serif",fontSize:22,
                  color:C.cream,marginBottom:8,lineHeight:1.2 }}>
                  New music drops regularly across all genres
                </div>
                <div style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:11,
                  color:C.sageTeal,marginBottom:16 }}>AI co-created with Suno · Free to use</div>
              ...
              <a href={YT} target="_blank" rel="noreferrer"
                style={{ display:"block",background:C.teal,color:C.cream,padding:"12px 20px",
                  ...
                  letterSpacing:"0.1em",fontWeight:500 }}>▶ Watch Latest on YouTube</a>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="shorts" style={{ padding:"40px 32px",maxWidth:1140,margin:"0 auto" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:16 }}>
          {SHORTS.map(item=><ShortCard key={item.title} item={item}/>)}
        </div>
      ...
      <section id="mission" style={{ padding:"80px 32px",maxWidth:1140,margin:"0 auto",
        ...
        <div style={{ position:"absolute",top:"50%",left:"50%",
          transform:"translate(-50%,-50%)",opacity:0.028,pointerEvents:"none" }}>
          <WaveSeal size={420} color={C.gold}/>
        </div>
        <div style={{ maxWidth:680,margin:"0 auto",textAlign:"center",position:"relative" }}>
          <div style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:11,
            color:C.sageTeal,letterSpacing:"0.2em",marginBottom:16 }}>THE SIGNAL</div>
          <h2 style={{ fontFamily:"'DM Serif Display',serif",
            fontSize:"clamp(28px,5vw,48px)",color:C.cream,lineHeight:1.2,marginBottom:20 }}>
            Music made to be <em style={{ color:C.gold }}>given away.</em>
          </h2>
          <p style={{ fontFamily:"'Georgia',serif",fontSize:16,
            color:C.dustyRose,lineHeight:1.85,marginBottom:16 }}>
          ...
          <p style={{ fontFamily:"'Georgia',serif",fontSize:16,
            color:C.mistTeal,lineHeight:1.85,marginBottom:40 }}>
          ...
          <div style={{ display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap" }}>
            ...
            <a href="#" style={{ background:"transparent",color:C.paleGold,
              ...
              border:`1px solid ${C.deepGold}55`,transition:"all 0.2s" }}
      ...
      <footer style={{ borderTop:`1px solid ${C.deepGold}18`,padding:"28px 32px",
        ...
        <div style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:10,
          color:C.roseMaroon,letterSpacing:"0.1em" }}>
