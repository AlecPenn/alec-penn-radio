...
function YouTubeRadio({ playing, onTrackChange, playlistId }) {
  ...
  const buildPlayer = (pid) => {
    ...
    // Random start point — feels like tuning into a live broadcast
    const randomIndex = Math.floor(Math.random()*15);
    const randomStart = Math.floor(Math.random()*1200) + 60; // 1-21 mins in
  ...
  useEffect(() => {
    ...
    if (window.YT?.Player) {
      buildPlayer(playlistId);
    } else {
      window.onYouTubeIframeAPIReady = () => buildPlayer(playlistId);
    }
    return () => { try { playerRef.current?.destroy?.(); } catch(e) {} };
  }, []);

  // When playlist changes — rebuild the player entirely
  useEffect(() => {
