...
function getPlaylist(g) { return PLAYLISTS[g] || PLAYLISTS["All"]; }
...
export default function AlecPennRadio() {
  ...
  const [activePlaylist,setActivePlaylist]=useState(PLAYLISTS["All"]);
                ...
                {[...GENRES,...GENRES].map((g,i)=>(
                  <span key={i} onClick={()=>handleGenre(g)}
                    style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:10,
                      color:g===genre?C.gold:C.deepGold+"55",letterSpacing:"0.14em",cursor:"pointer",transition:"color 0.2s" }}>{g}</span>
                ))}
              </div>
            </div>
        ...
        <div style={{
          ...
          <div style={{ position:"relative",zIndex:2,padding:mobile?"20px 16px 24px":"36px 36px 32px",
                ...
                {GENRES.slice(1).map(g=>(
                  <button key={g} onClick={()=>handleGenre(g)}
                    style={{ background:genre===g?C.teal:"transparent",
                      ...
                      transition:"all 0.15s",fontFamily:"inherit" }}>{g}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display:"flex",flexDirection:mobile?"row":"column",alignItems:"center",
              ...
              <FrequencyDial genre={genre} onChange={(g)=>handleGenre(g)}/>
              {!mobile&&(
