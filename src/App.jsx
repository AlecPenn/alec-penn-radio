...
const GENRES = [
  { name:"Lo-Fi",           id:"PLmNBbWRYKMiijl7vg_jK_uV_dG8L6Nruh" },
  { name:"Psychedelic Jazz",id:"PLmNBbWRYKMij00GHXkVmWKWOkjGHvkZUO" },
  { name:"Acoustic",        id:"PLmNBbWRYKMiilylZojuyYPs_t4akMjjwl" },
  { name:"Slow Jams",       id:"PLmNBbWRYKMiiqILu7MqJ1lXKOJwvx4f6u" },
  { name:"House",           id:"PLmNBbWRYKMig_sMtbb1pYP2onNhNM80Qi" },
  { name:"Neo-Classical",   id:"PLmNBbWRYKMiiJ_TVVlnOLJTV2V8sUdfCw" },
];
...
function YouTubeRadio({ playing, onTrackChange, playlistId }) {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      playerRef.current = new window.YT.Player(containerRef.current, {
        height:"1", width:"1",
        playerVars:{ listType:"playlist", list:playlistId, autoplay:0, controls:0, enablejsapi:1, origin:window.location.origin },
        events:{
          onReady:(e) => {
            readyRef.current = true;
            e.target.setShuffle(true);
            if (pendingRef.current) { e.target.playVideo(); pendingRef.current = false; }
          },
          onStateChange:(e) => {
    ...
    return () => { playerRef.current?.destroy?.(); readyRef.current = false; };
  },[]);
  ...
  useEffect(() => {
    ...
    playerRef.current.loadPlaylist({ listType:"playlist", list:playlistId, index:Math.floor(Math.random()*15) });
    playerRef.current.setShuffle?.(true);
    if (playing) playerRef.current.playVideo?.();
  },[playlistId]);

  useEffect(() => {
    if (!readyRef.current || !playerRef.current) { if (playing) pendingRef.current=true; return; }
...
function ParticleField({ active }) {
  ...
  useEffect(() => {
    ...
    const pts = Array.from({length:40},()=>({
      ...
      size:Math.random()*1.4+0.3,opacity:Math.random()*0.35+0.05,
...
function Oscilloscope({ active }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.lineWidth=1.5; ctx.shadowBlur=active?8:0; ctx.shadowColor=C.sageTeal;
      ctx.beginPath();
      for(let x=0;x<W;x++){
  ...
  return <canvas ref={canvasRef} width={180} height={44}
    style={{ display:"block",borderRadius:4,border:`1px solid ${active?C.sageTeal+"44":C.deepGold+"18"}`,background:"#030201",transition:"border-color 0.5s",maxWidth:"100%" }}/>;
}
...
function ShortCard({ item }) {
  ...
  return (
    <div onClick={()=>window.open(`https://youtube.com/watch?v=${videoId}`,"_blank")}
      ...
      <div style={{ color:C.cream,fontSize:12,fontWeight:500,lineHeight:1.3,overflow:"hidden",
        textOverflow:"ellipsis",display:"-webkit-box",WebkitLineClamp:2,WebkitBoxOrient:"vertical" }}>{title}</div>
...
function FrequencyDial({ genre, onChange }) {
  const angle=(GENRES.findIndex(g=>g.name===genre)/(GENRES.length-1))*180-90;
  return (
    <div style={{ display:"flex",flexDirection:"column",alignItems:"center",gap:8 }}>
      <div style={{ width:60,height:60,borderRadius:"50%",border:`2px solid ${C.gold}`,
        background:`radial-gradient(circle at 30% 30%,#1a1208,${C.black})`,
      ...
      <select value={genre} onChange={e=>onChange(e.target.value)}
        style={{ background:"transparent",border:`1px solid ${C.deepGold}`,color:C.paleGold,
          fontSize:10,padding:"3px 6px",borderRadius:4,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.06em" }}>
...
export default function AlecPennRadio() {
  ...
  useEffect(()=>{
    async function fetchData(){
      setLoadingYT(true);
      try {
        const [top,latest,sh]=await Promise.all([getTopVideos(),getLatestVideos(),getTopShorts()]);
        setTopVideos(top); setLatestVideo(latest[0]||null); setShorts(sh);
  ...
  return (
    <div style={{ minHeight:"100vh",background:C.black,color:C.cream,fontFamily:"'Georgia','Times New Roman',serif",cursor:mobile?"auto":"none",overflowX:"hidden" }}>
      ...
      <div style={{ position:"fixed",inset:0,pointerEvents:"none",zIndex:0,
        background:tuned?`radial-gradient(ellipse 80% 50% at 50% 0%,${C.teal}18 0%,transparent 70%)`:`radial-gradient(ellipse 60% 40% at 50% 0%,${C.deepGold}08 0%,transparent 70%)`,
      ...
      <nav style={{ position:"sticky",top:0,zIndex:50,background:`${C.black}f5`,backdropFilter:"blur(14px)",
        ...
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          ...
          <div style={{ width:6,height:6,borderRadius:"50%",marginLeft:4,
            ...
            boxShadow:tuned?`0 0 7px ${C.sageTeal}`:"none",transition:"background 0.5s,box-shadow 0.5s" }}/>
        </div>
        {!mobile&&(
          <>
                ...
                {[...GENRES,...GENRES].map((g,i)=>(
                  <span key={i} onClick={()=>handleGenre(g)}
                    style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:10,
                      color:g.name===genre?C.gold:C.deepGold+"55",letterSpacing:"0.14em",cursor:"pointer",transition:"color 0.2s" }}>{g.name}</span>
                ))}
              </div>
            </div>
            <div style={{ display:"flex",gap:20,alignItems:"center" }}>
              ...
              <a href={YT} target="_blank" rel="noreferrer"
                style={{ background:C.burgundy,color:C.cream,padding:"5px 14px",borderRadius:4,fontSize:11,
                  letterSpacing:"0.08em",textDecoration:"none",textTransform:"uppercase",
                  fontFamily:"'JetBrains Mono',monospace",border:`1px solid ${C.roseMaroon}` }}>YouTube ↗</a>
              <div style={{ display:"flex",gap:0,marginLeft:4 }}>
                {[{l:"─"},{l:"□"},{l:"✕"}].map((b,i)=>(
                  <div key={i} style={{ width:32,height:24,display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:i===2?11:13,color:C.warmGold,cursor:"default",borderLeft:`1px solid ${C.deepGold}22`,
        ...
        {mobile&&(
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            <a href={YT} target="_blank" rel="noreferrer"
              style={{ background:C.burgundy,color:C.cream,padding:"4px 12px",borderRadius:4,fontSize:10,
                letterSpacing:"0.08em",textDecoration:"none",fontFamily:"'JetBrains Mono',monospace",border:`1px solid ${C.roseMaroon}` }}>YT ↗</a>
            <button onClick={()=>setMenuOpen(o=>!o)}
              style={{ background:"transparent",border:`1px solid ${C.deepGold}44`,color:C.warmGold,
                padding:"6px 10px",borderRadius:4,cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",fontSize:14 }}>
              {menuOpen?"✕":"≡"}
            </button>
          </div>
        )}
      ...
      {mobile&&menuOpen&&(
        <div style={{ background:`${C.black}f8`,backdropFilter:"blur(14px)",borderBottom:`1px solid ${C.deepGold}22`,
          padding:"16px",display:"flex",flexDirection:"column",gap:12,position:"relative",zIndex:49 }}>
          {["#broadcast","#charts","#mission"].map((href,i)=>(
            <a key={href} href={href} onClick={()=>setMenuOpen(false)}
              style={{ color:C.warmGold,textDecoration:"none",fontFamily:"'JetBrains Mono',monospace",
                fontSize:12,letterSpacing:"0.12em",textTransform:"uppercase",padding:"8px 0",borderBottom:`1px solid ${C.deepGold}18` }}>
      ...
      {/* HERO */}
      <section id="broadcast" style={{ padding:mobile?`24px ${pad} 20px`:`48px ${pad} 40px`,maxWidth:1140,margin:"0 auto",position:"relative" }}>
        <div style={{
          ...
          <div style={{ position:"relative",zIndex:2,
            ...
            padding:mobile?"7px 14px":"9px 18px",display:"flex",alignItems:"center",justifyContent:"space-between" }}>
            <div style={{ display:"flex",alignItems:"center",gap:6 }}>
              <div style={{ width:6,height:6,borderRadius:"50%",background:tuned?C.sageTeal:C.dustyRose,
                animation:tuned?"on-air-blink 1.4s step-end infinite":"none" }}/>
              <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:tuned?C.sageTeal:C.dustyRose,letterSpacing:"0.12em" }}>
                {tuning?"TUNING...":tuned?"ON AIR":"STANDBY"}
              </span>
            </div>
            <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:mobile?8:11,color:C.warmGold,letterSpacing:"0.1em" }}>
              APR v2.0 — {timeStr}
            </span>
            {!mobile&&(
              <div style={{ display:"flex",gap:0 }}>
                {[{l:"─"},{l:"□"},{l:"✕"}].map((b,i)=>(
                  <div key={i} style={{ width:32,height:22,display:"flex",alignItems:"center",justifyContent:"center",
                    fontSize:i===2?10:12,color:C.warmGold,cursor:"default",borderLeft:`1px solid ${C.deepGold}22`,
                    userSelect:"none",transition:"background 0.1s" }}
                    onMouseEnter={e=>e.currentTarget.style.background=i===2?"#8B1A1A":`${C.deepGold}22`}
                    onMouseLeave={e=>e.currentTarget.style.background="transparent"}>{b.l}</div>
                ))}
          ...
          {/* Body */}
          <div style={{ position:"relative",zIndex:2,padding:mobile?"20px 16px 24px":"36px 36px 32px",
            display:"grid",gridTemplateColumns:mobile?"1fr":"1fr auto",gap:mobile?24:48,alignItems:"center" }}>
            ...
            <div>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:mobile?16:24,marginBottom:12 }}>
                ...
                <h1 style={{ fontFamily:"'Bebas Neue',sans-serif",fontSize:mobile?"clamp(40px,12vw,64px)":"clamp(50px,7vw,88px)",
                  color:C.gold,letterSpacing:"0.06em",lineHeight:0.88,animation:tuned?"freq-flicker 5s ease infinite":"none" }}>
              ...
              <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:20,flexWrap:"wrap" }}>
                <div style={{ display:"inline-flex",alignItems:"baseline",gap:4,background:"#030201",
                  border:`1px solid ${tuned?C.teal+"55":C.deepGold+"28"}`,borderRadius:6,
                  padding:mobile?"8px 14px":"10px 18px",transition:"border-color 0.7s" }}>
                  <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:mobile?24:32,fontWeight:500,
              ...
              <div style={{ display:"flex",alignItems:"center",gap:16,marginBottom:20,flexWrap:"wrap" }}>
                ...
                <button onClick={handleTuneIn} disabled={tuning}
                  style={{ background:tuned?C.burgundy:C.teal,border:"none",color:C.cream,
                    padding:mobile?"10px 24px":"12px 32px",borderRadius:6,fontSize:mobile?12:13,fontWeight:600,
                    letterSpacing:"0.12em",textTransform:"uppercase",cursor:tuning?"wait":"pointer",
                    fontFamily:"'JetBrains Mono',monospace",animation:tuned?"glow-pulse 2s ease infinite":"none",
                  ...
                  <div style={{ display:"flex",flexDirection:"column",gap:4 }}>
                    ...
                    {nowPlaying&&(
                      <div style={{ display:"flex",alignItems:"center",gap:6,maxWidth:240 }}>
                        <div style={{ width:5,height:5,borderRadius:"50%",background:C.sageTeal,animation:"pulse-dot 1s infinite",flexShrink:0 }}/>
                        <span style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:C.mistTeal,
                          letterSpacing:"0.06em",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{nowPlaying}</span>
                      </div>
                    )}
                  </div>
                )}
              ...
              <div style={{ display:"flex",gap:6,flexWrap:"wrap" }}>
                {GENRES.map(g=>(
                  <button key={g.name} onClick={()=>handleGenre(g)}
                    style={{ background:genre===g.name?C.teal:"transparent",
                      border:`1px solid ${genre===g.name?C.teal:C.deepGold+"44"}`,
                      color:genre===g.name?C.cream:C.warmGold,
                      padding:mobile?"5px 12px":"6px 16px",borderRadius:4,fontSize:10,
                      letterSpacing:"0.08em",textTransform:"uppercase",cursor:"pointer",
                      transition:"all 0.15s",fontFamily:"inherit" }}>{g.name}</button>
                ))}
              </div>
            </div>

            {/* Right panel */}
            <div style={{ display:"flex",flexDirection:mobile?"row":"column",alignItems:"center",
              gap:mobile?16:20,justifyContent:mobile?"space-between":"center",flexWrap:mobile?"wrap":"nowrap" }}>
              ...
              {!mobile&&(
                <div style={{ background:"#030201",border:`1px solid ${C.deepGold}22`,borderRadius:8,padding:"16px 20px",minWidth:172 }}>
                  {[{label:"SIGNAL",val:tuned?"████████░░":"███░░░░░░░",col:C.sageTeal},
                    ...
                    {label:"QUALITY",val:tuned?"█████████░":"████░░░░░░",col:C.gold}].map(m=>(
              ...
              <a href="#" style={{ display:"block",width:"100%",textDecoration:"none",
                ...
                padding:mobile?"12px 16px":"14px 20px",textAlign:"center",
                boxShadow:`0 4px 22px ${C.deepBurgundy}66`,transition:"box-shadow 0.2s" }}
      ...
      <section id="charts" style={{ padding:`40px ${pad}`,maxWidth:1140,margin:"0 auto" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:16 }}>
          <div style={{ border:`1px solid ${C.deepGold}28`,borderRadius:8,overflow:"hidden",background:"#0d0b09" }}>
            <div style={{ padding:"10px 14px",background:`linear-gradient(90deg,${C.deepBurgundy}88,transparent)`,
              borderBottom:`1px solid ${C.deepGold}28`,fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:C.warmGold,letterSpacing:"0.15em" }}>
              ▶ TOP 5 — ALL TIME VIEWS
            </div>
            {loadingYT?(
            ...
            <div style={{ padding:"10px 14px",textAlign:"center" }}>
              <a href={YT} target="_blank" rel="noreferrer"
                style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:C.sageTeal,textDecoration:"none" }}>Full chart on YouTube ↗</a>
            </div>
          </div>
          <div style={{ border:`1px solid ${C.teal}44`,borderRadius:8,overflow:"hidden",background:"#0d0b09",display:"flex",flexDirection:"column" }}>
            <div style={{ padding:"10px 14px",background:`linear-gradient(90deg,${C.teal}44,transparent)`,
              borderBottom:`1px solid ${C.teal}33`,fontFamily:"'JetBrains Mono',monospace",fontSize:9,color:C.sageTeal,letterSpacing:"0.15em",
            ...
            <div style={{ flex:1,padding:mobile?16:24,display:"flex",flexDirection:"column",justifyContent:"space-between",gap:16 }}>
              <div>
                <div style={{ fontFamily:"'DM Serif Display',serif",fontSize:mobile?16:20,color:C.cream,marginBottom:8,lineHeight:1.3 }}>
              ...
              <a href={latestId?`https://youtube.com/watch?v=${latestId}`:YT} target="_blank" rel="noreferrer"
                style={{ display:"block",background:C.teal,color:C.cream,padding:"11px 16px",borderRadius:6,textAlign:"center",
                  textDecoration:"none",fontFamily:"'JetBrains Mono',monospace",fontSize:11,letterSpacing:"0.1em",fontWeight:500 }}>
      ...
      <section id="shorts" style={{ padding:`40px ${pad}`,maxWidth:1140,margin:"0 auto" }}>
        ...
        <div style={{ textAlign:"center",marginTop:16 }}>
          <a href={`${YT}/shorts`} target="_blank" rel="noreferrer"
            style={{ fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:C.dustyRose,textDecoration:"none",letterSpacing:"0.08em" }}>
      ...
      <section id="mission" style={{ padding:mobile?`48px ${pad}`:`80px ${pad}`,maxWidth:1140,margin:"0 auto",borderTop:`1px solid ${C.deepGold}18`,position:"relative",overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:640,margin:"0 auto",textAlign:"center",position:"relative" }}>
          ...
          <div style={{ display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap" }}>
            <a href={YT} target="_blank" rel="noreferrer"
              style={{ background:C.burgundy,color:C.cream,padding:mobile?"12px 20px":"14px 32px",borderRadius:6,
                textDecoration:"none",fontFamily:"'JetBrains Mono',monospace",fontSize:mobile?11:12,
                letterSpacing:"0.1em",border:`1px solid ${C.roseMaroon}` }}>▶ Subscribe on YouTube</a>
            <a href="https://edensgate.art" target="_blank" rel="noreferrer"
              style={{ background:"transparent",color:C.paleGold,padding:mobile?"12px 20px":"14px 32px",borderRadius:6,
                textDecoration:"none",fontFamily:"'JetBrains Mono',monospace",fontSize:mobile?11:12,
                letterSpacing:"0.1em",border:`1px solid ${C.deepGold}55` }}>Eden's Gate ↗</a>
          </div>
        </div>
      </section>

      {/* FOOTER */}
      <footer style={{ borderTop:`1px solid ${C.deepGold}18`,padding:mobile?`20px ${pad}`:`28px ${pad}`,
